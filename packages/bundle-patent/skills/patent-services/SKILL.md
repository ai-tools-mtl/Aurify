...
7. **工具不可用时**（本会话未启用 MCP 服务）：明说"当前会话没有启用专利导出服务"，并给出启用方法——首选单文件：在 `~/.dsh/patent-services.yaml` 写 `mcp_enabled: true` 加 `mcp_project_dir: <patent-services 源码目录的绝对路径>`（或 `mcp_wheel: true`）——插件在进程启动时装载 MCP，保存后必须重启 dsh 进程才生效（桌面端：重启整个桌面壳，仅新开会话无效）；脚本化场景也可设环境变量 `DSH_PATENT_SERVICES_DIR`/`DSH_PATENT_SERVICES`。**不要**退回临时脚本拼改。配置变更后调 `patent_setup_check`（宿主侧工具）复验各通道——它区分「已装载」与「待重启」，显示「已装载」前不要当作工具可用。面向用户的回复一律简体中文。
